...
class LargeResultsSetPagination(pagination.PageNumberPagination):
    ...
    # Fix next/previous links inside sub-path-mounted reverse proxies in the KATSU context:

    def _get_katsu_absolute_uri(self):
        ...
        return urljoin(settings.KATSU_URL, full_path)

    def get_next_link(self):
        if settings.KATSU_URL is not None:
            # Monkey-patch rewrite build_absolute_uri
            self.request.build_absolute_uri = self._get_katsu_absolute_uri
