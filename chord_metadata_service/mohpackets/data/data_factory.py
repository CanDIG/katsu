...
class Dataset:
    ...
    def __init__(cls, program_count=4, donor_count=80, pd_count=80, specimen_count=80, sample_count=240,
                 treatment_count=160, exposure_count=40, followup_count=40, sys_therapy_count=320,
                 all_type_donor_count=2, null_type_donor_count=2):
        ...
        for i in range(0, program_count):
            ...
            for j in tqdm.tqdm(range(0, donors_per_program)):
                ...
                if j >= followups_per_program:
                    cls.Comorbidity.append(SynthComorbidityFactory.create(donor_uuid=donor_batch[j]))
                if exposure_start_index < j <= exposure_end_index:
                    cls.Exposure.append(SynthExposureFactory.create(donor_uuid=donor_batch[j]))

        logging.info(f"generating {null_type_donor_count} donors with all nulled fields")
        DonorFactory.reset_sequence(1)
        PrimaryDiagnosisFactory.reset_sequence(1)
        SpecimenFactory.reset_sequence(1)
        SampleRegistrationFactory.reset_sequence(1)
        ...
        ExposureFactory.reset_sequence(1)
        FollowUpFactory.reset_sequence(1)
        null_donors = NullSynthDonorFactory.create_batch(null_type_donor_count, program_id=cls.Program[0])
        cls.Donor.extend(null_donors)
        null_pds = NullSynthPrimaryDiagnosisFactory.create_batch(null_type_donor_count,
                                                                 donor_uuid=factory.Iterator(null_donors))
        cls.PrimaryDiagnosis.extend(null_pds)
        null_specimens=NullSynthSpecimenFactory.create_batch(null_type_donor_count,
                                                             primary_diagnosis_uuid=factory.Iterator(null_pds))
        cls.Specimen.extend(null_specimens)
        for i in range(0, null_type_donor_count):
            null_samples = NullSynthSampleRegistrationFactory.create_batch(samples_per_specimen,
                                                                           specimen_uuid=null_specimens[i])
            cls.SampleRegistration.extend(null_samples)
            null_treatments = NullSynthTreatmentFactory.create_batch(treatments_per_pd, primary_diagnosis_uuid=null_pds[i])
            cls.Treatment.extend(null_treatments)
            cls.Surgery.append(NullSynthSurgeryFactory.create(treatment_uuid=null_treatments[0]))
            cls.Radiation.append(NullSynthRadiationFactory.create(treatment_uuid=null_treatments[1]))
            ...
            if i >= followups_per_program:
                cls.Comorbidity.append(NullSynthComorbidityFactory.create(donor_uuid=null_donors[i]))
            if exposure_start_index < i <= exposure_end_index:
                cls.Exposure.append(NullSynthExposureFactory.create(donor_uuid=null_donors[i]))

        logging.info(f"generating {all_type_donor_count} donors with all fields filled in")
        DonorFactory.reset_sequence(1)
        PrimaryDiagnosisFactory.reset_sequence(1)
        SpecimenFactory.reset_sequence(1)
        SampleRegistrationFactory.reset_sequence(1)
        ...
        all_pds = AllSynthPrimaryDiagnosisFactory.create_batch(all_type_donor_count,
                                                               donor_uuid=factory.Iterator(all_donors))
        cls.PrimaryDiagnosis.extend(all_pds)
        all_specimens = AllSynthSpecimenFactory.create_batch(all_type_donor_count,
                                                             primary_diagnosis_uuid=factory.Iterator(all_pds))
        cls.Specimen.extend(all_specimens)
        for i in range(0, all_type_donor_count):
            all_samples = AllSynthSampleRegistrationFactory.create_batch(samples_per_specimen,
                                                                         specimen_uuid=all_specimens[i])
            cls.SampleRegistration.extend(all_samples)
            all_treatments = AllSynthTreatmentFactory.create_batch(treatments_per_pd, primary_diagnosis_uuid=all_pds[i])
            cls.Treatment.extend(all_treatments)
            cls.Surgery.append(AllSynthSurgeryFactory.create(treatment_uuid=all_treatments[0]))
            cls.Radiation.append(AllSynthRadiationFactory.create(treatment_uuid=all_treatments[1]))
            for j in range(0, treatments_per_pd):
